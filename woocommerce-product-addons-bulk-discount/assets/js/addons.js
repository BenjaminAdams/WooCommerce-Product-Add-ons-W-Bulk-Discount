		...
		$('form.cart').bind( 'woocommerce-product-addons-update', function() {
			...
			$cart.find('.addon').each(function() {
				...
				if ( $(this).is('.addon-custom-price') ) {
					...
					if(qty > 1 && addon_cost !=0)
					{
						discountedCost =  $(this).find('option:selected').data('discount-price');
					}
					...
					// var firstProduct = addon_cost 
					// var additionalCost = (qty-1) * discountedCost
					// total = parseFloat( total ) + parseFloat( addon_cost );
			...
			{
				var additionalCost = (qty-1) * discountedCost
				total = parseFloat( addon_cost ) +  parseFloat(additionalCost)
			} else
			{
				var singleItemCost = parseFloat( total ) + parseFloat( addon_cost );
				total = parseFloat( singleItemCost * qty );
			}
