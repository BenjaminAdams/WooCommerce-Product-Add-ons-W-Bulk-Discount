		...
		if ( ! empty( $cart_item['addons'] ) ) {
			...
			foreach ( $cart_item['addons'] as $addon )
				if ( $addon['price'] > 0 )
				{
					if($addon['discount'] > 0)
					{	
						$firstItemCost = $addon['price'];
						$avgPrice = ceil(($firstItemCost + (($qty -1) * $addon['discount'])) / $qty);
						$extra_cost += $avgPrice;
					}else {
				...
				switch ( $addon['type'] ) {
					...
					case "select" :
						...
						$cart_item_meta['addons'][] = array(
							...
							'discount' 	=> esc_attr( $chosen_option['discount'] )
						);
					...
					case "custom_price" :
						...
						foreach ( $addon['options'] as $option ) {
							...
							$label  = ! empty( $option['label'] ) ? trim( $option['label'] ) : trim( $addon['field-name'] );
						...
						case "custom_textarea" :
							...
							foreach ( $addon['options'] as $option ) {
								...
								if ( $addon['type'] == "custom_price" ) {
									...
									if ( $price < 0 ) {
										$woocommerce->add_error( sprintf( __( 'Invalid price entered for "%s".', 'woocommerce' ), $addon['field-name'] ) );
										return false;
									}
								}
							}
						...
						switch ( $addon['type'] ) {
							...
							case "custom_textarea" :
								...
								if ( ! empty( $option['min'] ) && ! empty( $posted ) && strlen( $posted ) < $option['min'] ) {
									$woocommerce->add_error( sprintf( __( 'The minimum allowed length for "%s - %s" is %s.', 'woocommerce' ), $addon['field-name'], $option['label'], $option['min'] ) );
									return false;
								}

								if ( ! empty( $option['max'] ) && ! empty( $posted ) && strlen( $posted ) > $option['max'] ) {
									$woocommerce->add_error( sprintf( __( 'The maximum allowed length for "%s - %s" is %s.', 'woocommerce' ), $addon['field-name'], $option['label'], $option['max'] ) );
									return false;
								}
							break;
							case "custom_price" :
								...
								if ( ! empty( $option['min'] ) && ! empty( $posted ) && $posted < $option['min'] ) {
									$woocommerce->add_error( sprintf( __( 'The minimum allowed amount for "%s - %s" is %s.', 'woocommerce' ), $addon['field-name'], $option['label'], $option['min'] ) );
									return false;
								}

								if ( ! empty( $option['max'] ) && ! empty( $posted ) && $posted > $option['max'] ) {
									$woocommerce->add_error( sprintf( __( 'The maximum allowed amount for "%s - %s" is %s.', 'woocommerce' ), $addon['field-name'], $option['label'], $option['max'] ) );
									return false;
								}
